...
impl ConfigTabState {
    ...
    pub fn draw(&mut self, ui: &Ui, timer_window_state: &mut TimerWindowState) {
        ...
        let timers_window_closure = || {
            ...
            if ui.checkbox("Stock Imgui Progress Bar", &mut timer_window_state.progress_bar.stock)  {
                let sender = TS_SENDER.get().unwrap();
                let event_send = sender.try_send(ControllerEvent::ProgressBarStyle(ProgressBarStyleChange::Stock(timer_window_state.progress_bar.stock)));
                drop(event_send);
            };
            ...
            if Slider
                ...
                let sender = TS_SENDER.get().unwrap();
                let event_send = sender.try_send(ControllerEvent::ProgressBarStyle(ProgressBarStyleChange::Height(timer_window_state.progress_bar.height)));
                drop(event_send);
            }
            let font_closure = || {
            ...
            if let Some(selection) = ComboBox
                ::new("Font")
                .preview_value(&timer_window_state.progress_bar.font.to_string())
                .build(ui, font_closure) {
                let sender = TS_SENDER.get().unwrap();
                let event_send = sender.try_send(ControllerEvent::ProgressBarStyle(ProgressBarStyleChange::Font(TextFont::from(selection.clone()))));
                drop(event_send);
            }
        };
        let timers_window = TreeNode
            ::new("Timers Window")
