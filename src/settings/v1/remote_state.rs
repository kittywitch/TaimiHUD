use {
    crate::{controller::ProgressBarStyleChange, render::TextFont, settings::{source::Source, GitHubSource, NeedsUpdate, RemoteSource}, timer::TimerFile, SETTINGS},
    anyhow::anyhow, async_compression::tokio::bufread::GzipDecoder, chrono::{DateTime, Utc}, futures::stream::{StreamExt, TryStreamExt}, nexus::imgui::Ui, reqwest::{Client, IntoUrl, Response}, serde::{de::DeserializeOwned, Deserialize, Serialize}, std::{
...
impl RemoteState {
    ...
    pub async fn load(&self) -> Vec<Arc<TimerFile>> {
        ...
        if let Some(path) = &self.installed_path {
            TimerFile::load_many(path, association, 100).await.expect("Could not load timer file for source {self.source}")
        } else {
            Default::default()
        }
    }
